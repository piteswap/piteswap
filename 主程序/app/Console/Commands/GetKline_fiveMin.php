                    ...
                    if ($result['hits']) {
                        ...
                        if ($last['itime'] === Robot::getNowTime($peroid) / 1000 || $last['itime'] < Robot::getNowTime($peroid) / 1000) {
                            ...
                            if ($last['itime'] < Robot::getNowTime($peroid) / 1000) {
                                $current = true;
                                $last_time = strtotime('+5 min', $last['itime']);
                            } else {
                                $last_time = $last['itime'];
                            }
                            $start = $this->getInfo($last_time, strtotime('+5 min', $last_time), $currency_name);
...
//                            if ($current) {
//                                $rkey = "market." . strtolower($currency_name) . "usdt.kline.".$peroid;
//
//                                $now_price = $faker->randomFloat(6, $obj['low'], $obj['high']);
//                                $hh = json_encode(['ch' => $rkey, 'ts' => Robot::getNowTime(), 'tick' => [
//                                    'id' => Robot::getNowTime() / 1000,
//                                    'open' => $obj['open'],
//                                    'close' => $now_price,
//                                    'low' => $obj['low'],
//                                    'high' => $obj['high'],
...
//                            }
                ...
                if ($result === false) {
                    ...
                    if (isset($result_quotation['hits'])) {
                        ...
                        $time = $body['itime'];
                        $start = $this->getInfo($time, strtotime('+5 min', $time), $currency_name);
