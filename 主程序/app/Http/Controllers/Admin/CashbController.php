...
class CashbController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $walletout = DB::table('users_wallet_out as uwo')
                        ->leftJoin('user_cash_info as uci','uwo.user_id','=','uci.user_id')
                        ->leftJoin('currency as c','uwo.currency','=','c.id')
                        ->leftJoin('users as u','uwo.user_id','=','u.id')
                        ->select('uwo.*','uci.bank_name','uci.bank_account','uci.real_name','c.name as currency_name', 'u.account_number', 'c.type as currency_type')
                        ->where('uwo.id',$id)
                        ->first();
        $use_chain_api = Setting::getValueByKey('use_chain_api', 0);
        $USDTRate = Setting::getValueByKey('USDTRate', 0);
        return view('admin.cashb.edit', ['wallet_out' => $walletout,'use_chain_api' => $use_chain_api,'USDTRate'=>$USDTRate]);
