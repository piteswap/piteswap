...
class CurrencyController extends Controller
{
    ...
    public function postAdd(Request $request)
    {
        ...
        //如果是这3种币则需要多个链的地址
        if(is_array($contract_address) && in_array(strtoupper($name),['BTC','USDT','ETH','EOS'])){
            $contract_address = json_encode($contract_address);
            if(strlen($contract_address)>255){
                return $this->error('地址过多');
            }
        }
