...
class AccountController extends Controller
{
    ...
    public function chargeMentionMoney(Request $request)
    {
     ...
     public function chongbi(Request $request)
    {
         $limit = $request->get('limit', 5);
         $user_id = Users::getUserId();
        $list = Db::table('charge_req')->where('uid', $user_id)
      ...
      public function tibi(Request $request)
    {
         $limit = $request->get('limit', 5);
         $user_id = Users::getUserId();
        $list = Db::table('users_wallet_out')->where('user_id', $user_id)
            ->join('currency','currency.id','=','users_wallet_out.currency')
            ->select(['users_wallet_out.*','currency.name'])
