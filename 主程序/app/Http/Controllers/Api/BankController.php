...
//        }
    ...
    public function dispatchInterest(){
      ...
      $res = LhDepositOrder::where('status',1)->where('start_at','<',date("Y-m-d"))
            ->where(function($query){
                $query->where('last_settle_time','<',date("Y-m-d"))
    ...
    public function product_info(Request $request){
        ...
        $list = DB::table('lh_deposit_config')->where('lh_deposit_config.id',$id)->join('currency','currency.id','=','currency_id')
        ->first(['currency.name as currency_name','currency.logo as currency_logo','lh_deposit_config.*']);
        return $this->success($list);
    }
    ...
    public function newDeposit(Request $request){
        ...
        if(!$config){
            return $this->error('项目不存在');//项目不存在
        }
        if($amount < $config->save_min){
            return $this->error('最少存入数量为：'.$config->save_min);//最少存入数量为：
        }
        //钱包
         $user_id = Users::getUserId();
         $legal = UsersWallet::where("user_id", $user_id)
        ...
        try{
            ...
            $order = LhDepositOrder::newOrder($user_id,$config->currency_id,$amount,$config->day,$config->interest_rate,$config->id);
    ...
    public function myDepositOrder(Request $request){
        ...
        $count = LhDepositOrder::where('user_id',$user_id)
            ->join('currency','currency.id','=','currency_id')
            ->where($where)->count();

        $res = LhDepositOrder::where('user_id',$user_id)
            ->join('currency','currency.id','=','lh_deposit_order.currency_id')
            ->leftjoin('lh_deposit_config','lh_deposit_config.id','=','lh_deposit_order.config_id')
            ->where($where)
            ->orderBy('lh_deposit_order.id','desc')
            ->skip($limit*($page-1))->take($limit)
            ->get(['currency.name as currency_name','currency.logo','lh_deposit_config.day','lh_deposit_order.id','lh_deposit_order.amount','lh_deposit_order.day_rate','lh_deposit_order.total_interest','lh_deposit_order.start_at','lh_deposit_order.end_at','lh_deposit_order.status','lh_deposit_order.is_cancel','lh_deposit_order.created_at','lh_deposit_order.updated_at']);
        return $this->success([
            'order_list' => $res,
            'total'=>$count
        ]);
    ...
    public function depositReport(Request $request){
         $user_id = Users::getUserId();
         $totalProfit = LhDepositOrder::where('user_id',$user_id)->sum('total_interest');
         $todayProfit = LhDepositOrderLog::where('user_id',$user_id)->where('created_at','>',date('Y-m-d 00:00:00'))->sum('interest_amount');
         $totalOrder = LhDepositOrder::where('user_id',$user_id)->where('status',1)->count();
         $totalDeposit =  LhDepositOrder::where('user_id',$user_id)->where('status',1)->sum('amount');
         
         return $this->success([
             'total_profit' => $totalProfit , //总收益
             'today_profit' => $todayProfit, //今日收益
             'total_order' => $totalOrder , //托管中的订单
             'total_deposit' => $totalDeposit //总托管资金
    ...
    public function depositLog(Request $request){
        $user_id = Users::getUserId();
        $page = $request->get('page')??1;
        $res = LhDepositOrderLog::join('currency','currency.id','=','currency_id')->where('user_id',$user_id)
        ->select(['currency.name as currency_name','lh_deposit_order_log.*'])->orderBy('lh_deposit_order_log.id','desc')
    ...
    public function cancelOrderNew(){
        ...
        if(!$order){
            return $this->error('找不到存单');  //找不到存单
        }
        $user_id = Users::getUserId();
        if($order->user_id != $user_id){
            return $this->error('非法操作');    //非法操作
        }
        if($order->status!=1){
            return $this->error('存单状态异常');    //存单状态异常
        }
        $legal = UsersWallet::where("user_id", $user_id)
