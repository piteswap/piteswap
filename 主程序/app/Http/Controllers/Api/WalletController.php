...
class WalletController extends Controller
{
    ...
    public function addAddress()
    {
        ...
        $netType = Input::get("netType", "");
        if (empty($user_id) || empty($id) || empty($address) || empty($netType)) {
            return $this->error("参数错误");
        }
        if(!in_array($notes,['USDT','ETH','BTC'])){
            return $this->error("参数错误");
        }
        
        $netArr = explode("|",env('WALLET_CH_'.$notes));
        if(!in_array($netType,$netArr)){
//            return $this->error("参数错误");
        }
        ...
        try {
            ...
            $currency_address->netType = $netType;
	...
	public function chargeReq(){
        ...
        if($currency_id!='9999'){   //9999为银行充值
            ...
            // }
            // $uacc[4] = $img;    //第5个是图片
            // $data['user_account'] = json_encode($uacc,320); 
            $rate = Db::table('settings')->where('key',"USDTRate")->first()->value;
            $data['daozhang_num'] =  sprintf("%.4f",$amount/$rate) ;//银行卡充值
            $data['is_bank'] = 1;//银行卡充值
        }
    ...
    public function changeWallet(Request $request)  //BY tian
    {
        ...
        try {
            ...
            if($from_account_log_type == 16 && $to_account_log_type == 15){
                $from_account_log_type = 1001;
                $to_account_log_type = 1002;
            }
            if($from_account_log_type == 15 && $to_account_log_type == 16){
                $from_account_log_type = 1002;
                $to_account_log_type = 1001;
            }
...
//资产划转
    ...
    { 
        ...
        if($result->items()){
            ...
            foreach ($result->items()  as $v ){
                if($lan == 'en'){
                    if($v->info == '期权划转币币'){
                        $info = 'Option transfer currency';
                    }elseif($v->info == '币币划转期权'){
                        $info = 'Currency transfer option';
                    }
                ...
                $list[] = [
                    ...
                    'before'=>	$v->before, 
                    'after'=>	$v->after, 
                    'transaction_info'=>	$v->transaction_info, 
                    'type_info_m'=>	$v->type_info_m, 
                    'type_info_e'=>	$v->type_info_e, 
                    'type_info_j'=>	$v->type_info_j, 
    ...
    public function getCurrencyInfo()
    {
        ...
        $data = [
            ...
            'max_number' => $currencyInfo->max_number,
    ...
    public function postWalletOut()
    {
        ...
        $code = Input::get('code','');
        $email = Users::getById(Users::getUserId())->email;
        $sys_code = Redis::get('code'.$email);
        if (env('NO_VERIFICATION')=='1' && empty($code) || env('NO_VERIFICATION')=='1' && $code != $sys_code) {  //验证
        ...
        //修补漏洞
        $rate = round($currencyInfo->rate*$number/100,4);
        ...
        try {
            ...
            $walletOut->is_bank = $tmp_currency_id == 9999?1:0; //提币出款类型 0币  1银行卡
    ...
    public function getWallettype(){
        $res['usdt'] = Setting::getValueByKey('usdt_switch')=="1"?true:false;
        $res['eth'] = Setting::getValueByKey('eth_switch')=="1"?true:false;
        $res['btc'] = Setting::getValueByKey('btc_switch')=="1"?true:false;
        $res['type'] = 'ok';
    ...
    public function getWalletAddressIn()
    {
        ...
        $currency_id = Input::get("currency", 5);
        $requestType = Input::get("requestType", 'moblie');
        if (empty($user_id) || empty($currency_id)) {
            return $this->error('参数错误');
        }
        ...
        if($currency_id==9999){
            ...
            $address = [
                "bkname"=>Setting::getValueByKey('BANK_NAME', ""),
                "bkcardnumer"=>Setting::getValueByKey('BANK_CODE', ""),
                "cardname"=>Setting::getValueByKey('BANK_USERNAME', "")
            ];
            return $this->success($address);
        }
        ...
        $legal = UsersWallet::where("user_id", $user_id)
         ...
         switch($currencyInfo->name){
         	case 'BTC':
         		$address = ['OMNI'=>config('app.btc')];
         		break;
 			case 'ETH':
 				$address = ['ERC20'=>config('app.eth')];
 				break;
 			case 'USDT':
 				$address = ['TRC20' => config('app.usdt_trc20'),'OMNI'=>config('app.usdt_omni'),'ERC20' => config('app.usdt_erc20')];
 				break;
        ...
        // if($address && in_array(strtoupper($currencyInfo->name),['BTC','USDT','ETH','EOS'])){
        ...
        //     $address = [
        //         "ERC20" =>"0xf37208C14Eb8B7975A513c319bdc5A5E604FD60B",
        //     ];
        // }elseif($currency_id == 3){
        //     $address = [
        //         "ERC20" =>"0xf37208C14Eb8B7975A513c319bdc5A5E604FD60B",
        //         "OMNI" =>"null",
        //         "TRC20" => "TGPHrwcnhNUZKvH3sRpozijd7KqG7Rx1jv"
        //     ];
        // }elseif($currency_id == 4){
        //     $address = [
        //         "ERC20" =>"0xf37208C14Eb8B7975A513c319bdc5A5E604FD60B",
        //         "ETH" => "0x3432c2253026Acd9f86b2E6F5B7c4453597508d7",
        //         "BTC" => "3QqWXnu9uarK98RymC1TrjZWB1ra6coeBx",
        //         "TRC20" => "TGPHrwcnhNUZKvH3sRpozijd7KqG7Rx1jv"
        //     ];
        // }else{
        //     $address = [
        ...
        //         "OMNI" =>"3L3b1ixw6FWWZuejJAtcYG7nL4kaFM4QgZ",
        //         "TRC20" => "TGPHrwcnhNUZKvH3sRpozijd7KqG7Rx1jv"
        //         // "bkname"=>"建设银行",
        //         // "bkcardnumer"=>"43323423423423",
        //         // "cardname"=>"李四"
