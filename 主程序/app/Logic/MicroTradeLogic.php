...
class MicroTradeLogic
{
    ...
    public static function addOrder($param)
    {
        ...
        try {
            ...
            throw_unless($seconds, new \Exception('秒数不在时间范围'));
            //检测数量是否在合法范围内
            /*
            ...
            */
            ...
            if (!preg_match('/^\d+$/', $number)) {
                throw new \Exception('The order quantity must be an integer');
            }
            ...
            if (bc_comp($currency->micro_max, $number) < 0  && bc_comp($currency->micro_max, 0) > 0) {
                throw new \Exception('The maximum number of orders cannot exceed:' . $currency->micro_max);
            }
            ...
            $result = change_wallet_balance($wallet, $balance_type, -$number, AccountLog::DEDUCT_PRINCIPAL_PLACING, '秒合约下单扣除本金');
            throw_unless($result === true, new \Exception($result));
            //扣手续费
            $fee = bc_div(bc_mul($number, $currency->micro_trade_fee), 100);
            $result = change_wallet_balance($wallet, $balance_type, -$fee, AccountLog::DEDUCT_PRINCIPAL_PLACING, '秒合约下单扣除' . $currency->micro_trade_fee . '%手续费');
    ...
    public static function close($match_id)
    {
        try {
            ...
            $micro_key = "micro_close".$match_id;  //redis key
            if(Redis::get($micro_key)){
                Log::info("秒合约 match_id: ".$match_id."正在执行");
                return false;
            }

            Redis::set($micro_key, true);  //写入缓存状态

            $currency_match = CurrencyMatch::find($match_id);
            if (!$currency_match) {
                Redis::set($micro_key, false);
                Log::info("秒合约平仓，此币不存在，match_id".$match_id);
                return false;
            }
            ...
            if (count($opened_orders) <= 0) {
                Redis::set($micro_key, false);
                Log::info("秒合约平仓， match_id: ".$match_id." 没有查询到可执行的订单");
                return false;
            }
            ...
            $ret = MicroOrder::whereIn('id', $opened_orders->pluck('id')->all())
                ->where('status', 1)
                ->update([
                    'status' => 2,
            ]);
            ...
            if(count($closing_oreders)<=0){
                Redis::set($micro_key, false);
                Log::info("秒合约平仓， match_id: ".$match_id."没有查询到正在平仓中的订单");
                return false;
            }
            ...
            foreach ($closing_oreders as $key => $value) {
                ...
                Log::info(" match_id:".$match_id."秒合约结算,此次操作条数:".count($closing_oreders));
                Log::info("【user_id】:".$value->user_id."  订单号:".$value->id);
            ...
            Redis::set($micro_key, false);  //写入缓存状态
        } catch (\Throwable $th) {
            Redis::set($micro_key, false);  //写入缓存状态
            Log::info($th->getFile() . ', Line:'. $th->getLine() . ':' . $th->getMessage());
        }
    }
