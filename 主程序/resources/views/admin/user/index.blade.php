    ...
    layui.use(['element', 'form', 'layer', 'table'], function () {
        ...
        var user_table = table.render({
            ...
            ,cols: [[
                ...
                ,{field:'parent_name', title:'上级代理', minWidth:100,templet:function(d){
                    if(d.parent_id<1){
                        return '-'
                    }else{
                ...
                ,{field:'last_login_ip', title:'上次登录IP', minWidth:200} 
                ,{field:'time', title:'注册时间', minWidth:200} 
        ...
        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'delete') { //删除
                layer.confirm('删除用户有可能导致系统数据异常，请非必须不要删除用户。确定要删吗？', function (index) {
                    //向服务端发送删除指令
                    $.ajax({
                        url: "/admin/user/del",
                        type: 'post',
