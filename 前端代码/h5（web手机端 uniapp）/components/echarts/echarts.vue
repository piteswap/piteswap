...
<script module="echarts" lang="renderjs">
	export default {
		...
		beforeDestroy:function(){
			//this.chart.off('datazoom')	//解绑dataZoom事件
		},
		methods: {
			/**
			 * 初始化echarts 
			...
			init() {
				...
				this.chart.on('datazoom',(event)=>{	//监听
					console.log('dz',event)
				   //this.$emit('eventDataZoom',event)
				   
				   if(event.batch){	//滚轮
				   	event.batch.forEach((item,index)=>{
				   		this.option.dataZoom[index].start = item.start
				   		this.option.dataZoom[index].end = item.end
				   	})
				   }else{//滑块
				   	const dz = this.option.dataZoom
				   	dz.forEach((item,index)=>{
				   		if(event.dataZoomId==item.id){
				   			this.option.dataZoom[index].start = event.start
				   			this.option.dataZoom[index].end = event.end
				   		}
				   	})
				   }
				   console.log('dzA',this.option.dataZoom)
