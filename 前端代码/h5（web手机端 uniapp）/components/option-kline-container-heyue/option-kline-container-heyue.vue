...
<script>
	...
	export default {
		name: "option-kline-container-heyue",
		props: {
			index_market: {
				type: String,
				default: "BTC/USDT"
			...
			*/
			...
			updateSeriesData: function(data) {
				...
				if (data.time < this.lastData.time) { //如果是初始化数据并且新消息时间<最后一条消息时间
					console.log(this.arrowSize, "HEYUE-ERROR TIME:", data.time, "<", this.lastData.time)
					return
				}
				...
				//console.log('ssscloseDetailHeyue')
				uni.$emit("closeDetailHeyue", {
					type: this.period,
