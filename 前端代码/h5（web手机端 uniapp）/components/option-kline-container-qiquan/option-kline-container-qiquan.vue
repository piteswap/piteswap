<template>
	<view class="option-kline-container" :data-symbol="index_market" :data-currentid="currentId">
		<view class="kline-tabs-wrap">
			<view v-for="(item,index) in tabList" :key="index" :data-current="index" :data-id="item.id"
				:data-value="item.value" :data-period="item.value" :data-type="item.chartType" :data-key="item.type"
				class="kline-tab-item" :class="index == subCurrent ? 'active' : ''"
				@click="LightweightCharts.onPeriodChange">
				<view class="kline-tab-label">{{$t(item.name)}}</view>
			</view>
				...
				<view class="kline-tab-item has-sub" @click.stop="showMore">
					...
					<view class="kline-tab-more-wrap bg-theme" v-if="isMoreShow">
						<view class="kline-tab-more-item" v-for="(sub,idx) in subItem" :key="idx"
							hover-class="kline-tab-more-item-hover" hover-stay-time="50" :data-current="idx"
							:data-value="sub.value" :data-id="sub.id" :data-period="sub.value"
							:data-type="sub.chartType" :data-key="sub.type" :data-isMore="true"
							@click="LightweightCharts.onPeriodChange">
							<text class="kline-more-label">{{$t(sub.name)}}</text>
						</view>
					</view>
				</view>

				<view class="kline-tab-item has-sub" @click="showIndex" v-if="currentId>0">
					<view class="kline-tab-label">
...
<script>
	...
	export default {
		name: "option-kline-container-qiquan",
		props: {
			index_market: {
				type: String,
				default: "BTC/USDT"
		...
		data() {
			return {
				...
				subItem: [{
					value: "1D",
					type: "1day",
					name: 'trade.kline.time_1day',
					period: "1D",
					chartType: 1,
					id: 5
				}, {
					name: 'trade.kline.time_1week',
					type: "1week",
					value: "1W",
					period: "1W",
					chartType: 1,
		...
		methods: {
			...
			*/
			...
			toAreaData: function(data) { //处理分时图数据
				if (Array.isArray(data)) {
					...
					var timestamp = Date.parse(new Date()) //推送来的数据使用新的时间戳
					return {
						time: timestamp,
						value: data.close
					}
			...
			fetchHistory: function() { //获取基础图表数据
				...
				this.$u.api.getNewTimeShar(params).then(res => {
					...
					if (this.currentId < 1) { //说明是分时图
						res = this.toAreaData(res)
					}
			...
			connect: function() { //socket.io连接websocket
				let that = this
				// console.log(this.socketApi)
				this.socket = this.SocketIO(this.socketApi);
				this.socket.on("connect", () => {
					console.log(111)
					this.socket.on('kline', function(msg) {
						if (msg.type == 'kline') {
							let obj = {}
					...
					this.socket.on('daymarket', function(msg) { //日线 更新图表顶部信息
			...
			updateSeriesData: function(data) {
				...
				if (data.time < this.lastData.time) { //如果是初始化数据并且新消息时间<最后一条消息时间
					console.log("QIQUAN-ERROR TIME:", data.time, "<", this.lastData.time)
					return
				}

				if (this.currentId < 1) { //说明是分时图
					data = this.toAreaData(data)
					//console.log("分时图data",sourceData,data)
				}
	...
	export default {
		...
		methods: {
			...
			init() { //初始化echarts
				...
				this.applyTimeScale("1min", nowCurrentId) //默认分时图

				if (nowCurrentId * 1 > 0) { //常规K线图
					...
					candleSeries = chart.addAreaSeries({
						topColor: 'rgba(45,226,180, 0.8)',
						bottomColor: 'rgba(45,226,180, 0)',
						lineColor: '#2DE2B4',
						lineWidth: 2,
						crosshairMarkerVisible: false, //显示十字线焦点球
						crosshairMarkerRadius: 4,
						crosshairMarkerBorderColor: '#2DE2B4',
						crosshairMarkerBackgroundColor: '#fff',
						lastPriceAnimation: 1 //开启最后价格动画
					})
					chart.applyOptions({
						localization: {
							timeFormatter: (t) => {
								return LightweightCharts.isBusinessDay(t),
									this.dateFormat(t, "H:i:s")
							}
						},
						layout: {
							backgroundColor: "#1B2C42",
				...
				if ("series" == this.series.type) {
					...
					if (chart) {
						...
						return candleSeries.setData(this.series.data),
							maStatus ? this.createMA() : bollStatus && this.createBoll(),
					...
					setTimeout((function() {
						...
						maStatus ? that.createMA() : bollStatus && that.createBoll()
			...
			applyTimeScale: function(t, currentId) { //X轴时间线配置(用于切换了数据类型时调用)
				let that = this
				chart && ("1min" == t && 0 == currentId ? chart.applyOptions({ //分时图
					timeScale: {
						timeVisible: true,
						secondsVisible: true,
						tickMarkFormatter: function(t) {
							return that.dateFormat(t, "H:i:s")
						}
					}
				}) : "1day" == t ? chart.applyOptions({
					timeScale: {
						timeVisible: true,
						secondsVisible: true,
						tickMarkFormatter: function(t) {
			...
			onPeriodChange: async function(e, i) {
				...
				if (currentId == nowCurrentId) {
					//console.log('相同点击',currentId,nowCurrentId)
					return //避免重复点击相同的K线时间点刷新
				} else if (currentId == 0 && nowCurrentId > 0 && bollStatus == true) { //如果是进入分时图
					//console.log('请关闭boll',bollStatus)

					this.closeBoll()
					i.callMethod("setSubActive", null)

					await new Promise(function(resolve) { //执行一个阻塞以防boll报错
				...
				i.callMethod("setCurrentId", currentId)
				...
				this.applyTimeScale(r, current)

				if (currentId == 0 || nowCurrentId == 0) { //其中一个为0说明是切换到分时图 或从分时图切出

					if (candleSeries) { //切换图表时清空图表
						//console.log('删除之前的实例',lineMA5)
						chart.removeSeries(candleSeries), candleSeries = null
						lineMA5 && chart.removeSeries(lineMA5), lineMA5 = null
						lineMA10 && chart.removeSeries(lineMA10), lineMA10 = null
						lineMA30 && chart.removeSeries(lineMA30), lineMA30 = null
						w.up && chart.removeSeries(w.up), w.up = null
						w.middle && chart.removeSeries(w.middle), w.middle = null
						w.down && chart.removeSeries(w.down), w.down = null
					}
