<template>
	<view class="dark" v-if="isLogin">
		<lw-navbar :border-bottom="false" :title='$t("template.fundFinancial.default[2]")'></lw-navbar>
		<view>
			<view class="content darkStyle">
				<view class="header top-card" style="z-index:102">
					<view class="headerTop" style="z-index:102">
						<view class="left">
							<text class="title">{{(Number(financeDetail.total_deposit) || 0).toFixed(4)}}</text>
							<text>{{$t("template.fundFinancial.detail[0]")}}(USDT)</text>
						</view>
						<view class="rig">
							<view class="ordersBtn rig-btn" @click="gotoNext('/pages/finance/ordersPage')">
								<text>{{$t("template.fundFinancial.detail[2]")}}</text>
							</view>
							<view class="ruleBtn rig-btn" @click="gotoNext('/pages/finance/rulePage')">
								<text>{{$t("template.fundFinancial.detail[3]")}}</text>
							</view>
						</view>
					</view>
					...
					<view class="headerBottom">
						<view class="left">
							<text>{{(Number(financeDetail.today_profit) || 0).toFixed(4)}}</text>
							<text style="font-size: 26rpx;">{{$t("template.fundFinancial.detail[4]")}}</text>
						</view>
						<view class="line"></view>
						<view class="midd">
							<text>{{(Number(financeDetail.total_profit) || 0).toFixed(4)}}</text>
							<text style="font-size: 26rpx;">{{$t("template.fundFinancial.detail[5]")}}</text>
						</view>
						<view class="line"></view>
						<view class="rig">
							<text>{{financeDetail.total_order || 0}}</text>
							<text style="font-size: 26rpx;">{{$t("template.fundFinancial.detail[6]")}}</text>
						</view>
					</view>
				</view>
				<view class="content-main">
					...
					<view class="list-view-wraps" v-if="currencyTypesList.length > 0">
						<view class="listview" v-for="(item,index) in currencyTypesList" :key="index" v-if="item.faxing == 0">
							<view class="rowTop">
								...
								<view class="buyBtn" @click="gotoBuy(item)">{{$t("template.fundFinancial.labels[4]")}}</view>
							</view>
							
							<view class="rowcenter row-line">
								<view class="left">{{$t("template.fundFinancial.labels[1]")}}</view>
								<view class="center">{{$t("template.fundFinancial.labels[2]")}}%</view>
								<view class="rig">{{$t("template.fundFinancial.labels[3]")}}</view>
							</view>
							<view class="rowBottom row-line">
								<view class="left">{{item.save_min*1}}</view>
								<view class="center">{{(item.interest_rate*1).toFixed(2)}}%</view>
								<view class="rig">{{$t("template.fundFinancial.labels[5]").replace("{days}", item.day)}}</view>
							</view>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		gotoBuy: function(t) {
			...
			this.$u.route({url:this.path.buyPage,params:e})
		},
