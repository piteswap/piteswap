    ...
    <div class="main-register">
      <div class="flex" style="max-width: 1200px; margin: 0 auto">
        <div class="reg-content" style="min-height: 700px; width: 620px">
          <!-- <div class="nav-list">
            <div :class="['nav-item', !isMb && 'active']" @click="isMb = false">选择邮箱</div>
            <div :class="['nav-item', isMb && 'active']" @click="isMb = true">选择手机号</div>
          </div> -->

          <div class="step-one">
           
			<div class="pwd-box">
				<div class="tip">{{ $t("security.oldlogjymm") }}</div>
				<input
				  type="password"
				  v-model="oldpwd"
				  class="pwd-input"
				  :placeholder="$t('security.oldlogjymm')"
				/>
				
			  </div>
			  <div class="pwd-box">
			    <div class="tip">{{ $t("security.newjymm") }}</div>
			    <input
			      type="password"
			      v-model="pwd"
			      class="pwd-input"
			      :placeholder="$t('security.newilogjymm')"
			    />
			  			
			  </div>
			  <div class="pwd-box">
			    <div class="tip">{{ $t("security.isnewjymm") }}</div>
			    <input
			      type="password"
			      v-model="repwd"
			      class="pwd-input"
...
export default {
  ...
  methods: {
	...
	resetphone(){
		...
		if (password == "" || re_password == "" || oldpassword == '') {
		  return;
		}  else {
		  this.$http({
